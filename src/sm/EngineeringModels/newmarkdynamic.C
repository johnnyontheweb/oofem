...
#include "sm/EngineeringModels/newmarkdynamic.h"
...
namespace oofem {
REGISTER_EngngModel(NewmarkDynamic);

NewmarkDynamic :: NewmarkDynamic(int i, EngngModel *_master) : StructuralEngngModel(i, _master),
...
NewmarkDynamic :: ~NewmarkDynamic()
{
}


NumericalMethod *NewmarkDynamic :: giveNumericalMethod(MetaStep *mStep)
{
    if ( mStep == NULL ) {
...
void
NewmarkDynamic :: updateAttributes(MetaStep *mStep)
{
    ...
    deltaT = 1.0;
    IR_GIVE_OPTIONAL_FIELD(ir, deltaT, _IFT_NewmarkDynamic_deltat);
    if ( deltaT < 0. ) {
        OOFEM_ERROR("deltaT < 0");
    }

    eta = 0;
    IR_GIVE_OPTIONAL_FIELD(ir, eta, _IFT_NewmarkDynamic_eta);

    delta = 0;
    IR_GIVE_OPTIONAL_FIELD(ir, delta, _IFT_NewmarkDynamic_delta);
}


void
NewmarkDynamic :: initializeFrom(InputRecord &ir)
{
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, nonlocalStiffnessFlag, _IFT_NewmarkDynamic_nonlocstiff);
    ...
    OOFEM_LOG_INFO( "Newmark-beta method\n" );
    IR_GIVE_OPTIONAL_FIELD( ir, gamma, _IFT_NewmarkDynamic_gamma );
    IR_GIVE_OPTIONAL_FIELD( ir, beta, _IFT_NewmarkDynamic_beta );

    MANRMSteps = 0;
    IR_GIVE_OPTIONAL_FIELD(ir, MANRMSteps, _IFT_NRSolver_manrmsteps);

    secOrder = false;
    IR_GIVE_OPTIONAL_FIELD(ir, secOrder, _IFT_NewmarkDynamic_secondOrder);
    ...
    if ( isParallel() ) {
        ...
        if ( ir.hasField(_IFT_NewmarkDynamic_nonlocalext) ) {
            nonlocalExt = 1;
...
double NewmarkDynamic :: giveUnknownComponent(ValueModeType mode, TimeStep *tStep, Domain *d, Dof *dof)
{
...
TimeStep *NewmarkDynamic :: giveNextStep()
{
...
void NewmarkDynamic :: solveYourself()
{
    if ( commInitFlag ) {
...
void
NewmarkDynamic :: solveYourselfAt(TimeStep *tStep)
{
    if ( commInitFlag ) {
 #ifdef __VERBOSE_PARALLEL
        // Force equation numbering before setting up comm maps.
...
void
NewmarkDynamic :: terminate(TimeStep *tStep)
{
...
void 
NewmarkDynamic :: initializeYourself(TimeStep *tStep)
{
    ...
    if ( tStep->isTheFirstStep() && initFlag ) {
        ...
        TimeStep *stepWhenIcApply = new TimeStep(giveNumberOfTimeStepWhenIcApply(), this, 0, -deltaT, deltaT, 0);
...
void
NewmarkDynamic :: proceedStep(int di, TimeStep *tStep)
{
...
void
NewmarkDynamic :: determineConstants(TimeStep *tStep)
{
...
void NewmarkDynamic :: updateYourself(TimeStep *tStep)
{
    totIterations = 0;
...
void NewmarkDynamic :: updateSolution(FloatArray &solutionVector, TimeStep *tStep, Domain *d)
{
...
void NewmarkDynamic :: updateInternalRHS(FloatArray &answer, TimeStep *tStep, Domain *d, FloatArray *eNorm)
{
#ifdef VERBOSE
...
void NewmarkDynamic :: updateMatrix(SparseMtrx &mat, TimeStep *tStep, Domain *d)
{
...
void NewmarkDynamic :: updateComponent(TimeStep *tStep, NumericalCmpn cmpn, Domain *d)
//
...
void
NewmarkDynamic :: printOutputAt(FILE *file, TimeStep *tStep)
{
...
void
NewmarkDynamic :: printDofOutputAt(FILE *stream, Dof *iDof, TimeStep *tStep)
{
...
void NewmarkDynamic :: saveContext(DataStream &stream, ContextMode mode)
{
...
void NewmarkDynamic :: restoreContext(DataStream &stream, ContextMode mode)
{
    contextIOResultType iores;
...
void
NewmarkDynamic :: updateDomainLinks()
{
...
void
NewmarkDynamic :: assemble(SparseMtrx &answer, TimeStep *tStep, const MatrixAssembler &ma,
...
#ifdef TIME_REPORT
    timer.stopTimer();
    OOFEM_LOG_DEBUG( "NewmarkDynamic info: user time consumed by assembly: %.2fs\n", timer.getUtime() );
#endif
}

#ifdef __OOFEG
void
NewmarkDynamic :: showSparseMtrxStructure(int type, oofegGraphicContext &gc, TimeStep *tStep)
{
    Domain *domain = this->giveDomain(1);
    CharType ctype;
...
void
NewmarkDynamic :: timesMtrx(FloatArray &vec, FloatArray &answer, CharType type, Domain *domain, TimeStep *tStep)
{
...
int
NewmarkDynamic :: estimateMaxPackSize(IntArray &commMap, DataStream &buff, int packUnpackType)
{
...
#ifdef __PARALLEL_MODE
LoadBalancer *
NewmarkDynamic :: giveLoadBalancer()
{
    if ( lb ) {
        return lb.get();
    }
...
LoadBalancerMonitor *
NewmarkDynamic :: giveLoadBalancerMonitor()
{
    if ( lbm ) {
        return lbm.get();
    }
...
void
NewmarkDynamic :: packMigratingData(TimeStep *tStep)
{
    Domain *domain = this->giveDomain(1);
...
void
NewmarkDynamic :: unpackMigratingData(TimeStep *tStep)
{
    Domain *domain = this->giveDomain(1);
    int ndofman = domain->giveNumberOfDofManagers(), _eq;
