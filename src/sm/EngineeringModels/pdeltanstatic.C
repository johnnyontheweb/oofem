...
namespace oofem {
REGISTER_EngngModel(PdeltaNstatic);

PdeltaNstatic :: PdeltaNstatic(int i, EngngModel *_master) : LinearStatic(i, _master),
...
NumericalMethod *PdeltaNstatic :: giveNumericalMethod(MetaStep *mStep)
{
    ...
    int _val = 0;
    IR_GIVE_OPTIONAL_FIELD( ( mStep->giveAttributesRecord() ), _val, _IFT_PdeltaNstatic_controlmode );
    ...
    PdeltaNstatic_controlType mode = ( PdeltaNstatic_controlType ) _val;
...
PdeltaNstatic :: updateAttributes(MetaStep *mStep)
{
     ...
     * double factor;
     *
     * printf ("PdeltaNstatic: fixed load level");
    ...
    IR_GIVE_OPTIONAL_FIELD(ir, _val, _IFT_PdeltaNstatic_controlmode);
    ...
    this->controlMode = ( PdeltaNstatic_controlType ) _val;
    ...
    deltaT = 1.0;
    IR_GIVE_OPTIONAL_FIELD(ir, deltaT, _IFT_PdeltaNstatic_deltat);
    if ( deltaT < 0. ) {
...
double PdeltaNstatic :: giveUnknownComponent(ValueModeType mode, TimeStep *tStep, Domain *d, Dof *dof)
...
// This function translates this request to numerical method language
{
	...
	case VM_Acceleration:
		...
		return 0;
    default:
		OOFEM_WARNING("Unknown is of undefined ValueModeType for this problem");
		return 0;
    }
...
TimeStep *PdeltaNstatic :: giveSolutionStepWhenIcApply(bool force)
{
  if ( master && (!force)) {
...
TimeStep *PdeltaNstatic :: giveNextStep()
{
...
void PdeltaNstatic :: solveYourself()
{
    if ( this->isParallel() ) {
...
void
PdeltaNstatic :: solveYourselfAt(TimeStep *tStep)
{
    proceedStep(1, tStep);
}
...
void
PdeltaNstatic :: updateLoadVectors(TimeStep *tStep)
{
...
void
PdeltaNstatic :: proceedStep(int di, TimeStep *tStep)
{
    //
    ...
    //
	//if (currentStep->giveTargetTime() == 0) 
	//	this->giveNextStep();
    if ( initFlag ) {
        //
...
void
PdeltaNstatic :: updateDomainLinks()
{
...
void
PdeltaNstatic :: assemble(SparseMtrx &answer, TimeStep *tStep, const MatrixAssembler &ma,
...
#ifdef TIME_REPORT
    timer.stopTimer();
    OOFEM_LOG_DEBUG( "PdeltaNstatic: User time consumed by assembly: %.2fs\n", timer.getUtime() );
#endif
}
...
#ifdef __OOFEG
void
PdeltaNstatic :: showSparseMtrxStructure(int type, oofegGraphicContext &gc, TimeStep *tStep)
{
...
void
PdeltaNstatic :: computeExternalLoadReactionContribution(FloatArray &reactions, TimeStep *tStep, int di)
{
...
void
PdeltaNstatic :: assembleIncrementalReferenceLoadVectors(FloatArray &_incrementalLoadVector,
...
int
PdeltaNstatic :: estimateMaxPackSize(IntArray &commMap, DataStream &buff, int packUnpackType)
{
...
#ifdef __PARALLEL_MODE
LoadBalancer *
PdeltaNstatic :: giveLoadBalancer()
{
    if ( lb ) {
        return lb;
    }
...
LoadBalancerMonitor *
PdeltaNstatic :: giveLoadBalancerMonitor()
{
    if ( lbm ) {
        return lbm;
    }
...
void
PdeltaNstatic :: packMigratingData(TimeStep *tStep)
{
    Domain *domain = this->giveDomain(1);
...
void
PdeltaNstatic :: unpackMigratingData(TimeStep *tStep)
{
    Domain *domain = this->giveDomain(1);
    int ndofman = domain->giveNumberOfDofManagers();
