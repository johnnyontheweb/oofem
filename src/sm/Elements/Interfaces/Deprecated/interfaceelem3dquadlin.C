...
#include "interfaceelem3dquadlin.h"
...
namespace oofem {
REGISTER_Element(InterfaceElement3dQuadLin);

FEI2dQuadLin InterfaceElement3dQuadLin :: interpolation(1, 2);

InterfaceElement3dQuadLin :: InterfaceElement3dQuadLin(int n, Domain *aDomain) :
...
void
InterfaceElement3dQuadLin :: computeBmatrixAt(GaussPoint *gp, FloatMatrix &answer, int li, int ui)
//
...
void
InterfaceElement3dQuadLin :: computeGaussPoints()
// Sets up the array of Gauss Points of the receiver.
{
    if ( integrationRulesArray.size() == 0 ) {
		...
		integrationRulesArray[0]->SetUpPointsOnSquare(4, _3dInterface);
    }
}


int
InterfaceElement3dQuadLin :: computeGlobalCoordinates(FloatArray &answer, const FloatArray &lcoords)
{
    FloatArray n;
...
bool
InterfaceElement3dQuadLin :: computeLocalCoordinates(FloatArray &answer, const FloatArray &gcoords)
{
...
double
InterfaceElement3dQuadLin :: computeVolumeAround(GaussPoint *gp)
// Returns the length of the receiver. This method is valid only if 1
// Gauss point is used.
{
	double determinant, weight, thickness, volume;
    // first compute local nodal coordinates in element plane
    std::vector< FloatArray > lncp(4);
    FloatMatrix lcs(3, 3);
    this->computeLCS(lcs);
    for ( int i = 1; i <= 4; i++ ) {
...
void
InterfaceElement3dQuadLin :: computeStressVector(FloatArray &answer, const FloatArray &strain, GaussPoint *gp, TimeStep *tStep)
{
...
void
InterfaceElement3dQuadLin :: computeConstitutiveMatrixAt(FloatMatrix &answer, MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep)
{
...
IRResultType
InterfaceElement3dQuadLin :: initializeFrom(InputRecord *ir)
{
...
void
InterfaceElement3dQuadLin :: giveDofManDofIDMask(int inode, IntArray &answer) const
{
    answer = {D_u, D_v, D_w};
}
...
void
InterfaceElement3dQuadLin :: computeLCS(FloatMatrix &answer)
{
...
bool
InterfaceElement3dQuadLin :: computeGtoLRotationMatrix(FloatMatrix &answer)
{
    ...
    for ( int i = 0; i < 8; i++ ) {
        for ( int j = 1; j <= 3; j++ ) {
            answer.at(i * nr + 1, i * nr + j) = lcs.at(3, j);
            answer.at(i * nr + 2, i * nr + j) = lcs.at(1, j);
            answer.at(i * nr + 3, i * nr + j) = lcs.at(2, j);
        }
    }
...
#ifdef __OOFEG
void InterfaceElement3dQuadLin :: drawRawGeometry(oofegGraphicContext &gc, TimeStep *tStep)
{
    GraphicObj *go;
...
void InterfaceElement3dQuadLin :: drawDeformedGeometry(oofegGraphicContext &gc, TimeStep *tStep, UnknownType type)
{ }


void InterfaceElement3dQuadLin :: drawScalar(oofegGraphicContext &gc, TimeStep *tStep)
{ }
